...
from lib.nn.sync_batchnorm.batchnorm import SynchronizedBatchNorm1d, SynchronizedBatchNorm2d, convert_model
from lib.nn.sync_batchnorm.replicate import patch_replication_callback
from lib.nn.sync_batchnorm.replicate import DataParallelWithCallback
from lib.nn.sync_batchnorm.unit_test import TorchTestCase
...
class SyncTestCase(TorchTestCase):
    ...
    def testSyncBatchNormNormalTrain(self):
        ...
        sync_bn = SynchronizedBatchNorm1d(10, momentum=0.1)
    ...
    def testSyncBatchNormSyncTrain(self):
        ...
        sync_bn = SynchronizedBatchNorm1d(10, eps=1e-5, affine=False, momentum=0.1)
    ...
    def testSyncBatchNorm2DSyncTrain(self):
        ...
        sync_bn = SynchronizedBatchNorm2d(10, momentum=0.1)
        sync_bn = DataParallel(sync_bn, device_ids=[0,1])
        patch_replication_callback(sync_bn)
        #sync_bn = DataParallelWithCallback(sync_bn, device_ids=[0, 1])
    ...
    def testConvertSyncBatchNorm2DSyncTrain(self):
        bn = nn.BatchNorm2d(10)
        sync_bn = convert_model(nn.BatchNorm2d(10))
        sync_bn = DataParallel(sync_bn, device_ids=[0,1])
        patch_replication_callback(sync_bn)
        #sync_bn = DataParallelWithCallback(sync_bn, device_ids=[0, 1])
