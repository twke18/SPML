...
def main():
  """Generate pseudo labels by softmax classifier, random walk and CRF.
  """
  ...
  with torch.no_grad():
    for data_index in tqdm(range(len(test_dataset))):
      ...
      semantic_probs = torch.cat(semantic_probs, dim=0)
      semantic_probs = torch.mean(semantic_probs, dim=0)
