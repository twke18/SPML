...
# the baseline method with SPML on DensePose using 
...
BACKBONE_TYPES=panoptic_pspnet_101
EMBEDDING_DIM=32
...
DATAROOT=/home/twke/data/MSCOCO
PRETRAINED=./snapshots/imagenet/trained/resnet-101-cuhk.pth
TRAIN_DATA_LIST=datasets/densepose/panoptic_${TRAIN_SPLIT}_points_pmi0.1_256.txt
TEST_DATA_LIST=datasets/densepose/panoptic_${INFERENCE_SPLIT}.txt
MEMORY_DATA_LIST=datasets/densepose/panoptic_${TRAIN_SPLIT}_sub.txt
...
if [ ${IS_CONFIG_EMB} -eq 1 ]; then
  ...
  sed -e "s/TRAIN_SPLIT/${TRAIN_SPLIT}/g"\
    ...
    configs/densepose_template.yaml > ${SNAPSHOT_DIR}/config_emb.yaml
...
if [ ${IS_TRAIN_EMB} -eq 1 ]; then
  python3 pyscripts/train/train_densepose.py\
    --data_dir ${DATAROOT}\
  ...
  python3 pyscripts/benchmark/benchmark_by_mIoU.py\
    ...
    --gt_dir ${DATAROOT}/segcls/densepose/gray\
    --num_classes 15
...
if [ ${IS_CONFIG_CLASSIFIER} -eq 1 ]; then
  ...
  sed -e "s/TRAIN_SPLIT/${TRAIN_SPLIT}/g"\
    ...
    configs/densepose_template.yaml > ${SNAPSHOT_DIR}/config_classifier.yaml
...
if [ ${IS_ANNOTATION_1} -eq 1 ]; then
  ...
  python3 pyscripts/inference/pseudo_denseposerw_crf.py\
    --data_dir ${DATAROOT}\
    --data_list ${TRAIN_DATA_LIST}\
    --snapshot_dir ${SNAPSHOT_DIR}/stage1\
  ...
  sed -e "s#segcls\/densepose_points\/gray#`pwd`\/${SNAPSHOT_DIR}\/stage1\/pseudo_labels\/${TRAIN_SPLIT}_cam_rw\/semantic_gray#g"\
    -e "s#images#${DATAROOT}\/images#g"\
    -e "s#seginst#${DATAROOT}\/seginst#g"\
...
if [ ${IS_TRAIN_CLASSIFIER_1} -eq 1 ]; then
  python3 pyscripts/train/train_densepose_classifier.py\
    --data_dir ${HOME}\
