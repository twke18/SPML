...
# image tag annotations. Users could also modify from this
...
SEM_ANN_LOSS_WEIGHT=0.3
SEM_OCC_LOSS_WEIGHT=0.3
...
IS_ANNOTATION_2=1
IS_TRAIN_CLASSIFIER_2=1
IS_INFERENCE_CLASSIFIER_2=1
IS_BENCHMARK_CLASSIFIER_2=1
...
TRAIN_DATA_LIST=datasets/voc12/seam_${TRAIN_SPLIT}_a6_th0.2_hed.txt
...
if [ ${IS_ANNOTATION_1} -eq 1 ]; then
  ...
  python3 pyscripts/inference/pseudo_camrw_crf.py\
    --data_dir ${DATAROOT}\
    --data_list ${TRAIN_DATA_LIST}\
    --snapshot_dir ${SNAPSHOT_DIR}/stage1\
    --save_dir ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_rw\
    --cam_dir /home/twke/repos/SEAM/outputs/train+/cam\
  ...
  python3 pyscripts/benchmark/benchmark_by_instance.py\
    --pred_dir ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_rw/semantic_gray\
    --gt_dir ${DATAROOT}/VOC2012/segcls\
    --inst_dir ${DATAROOT}/VOC2012/seginst\
    --num_classes 21

  sed -e "s#VOC2012\/cam\/seam_a6_th0.2#`pwd`\/${SNAPSHOT_DIR}\/stage1\/pseudo_labels\/${TRAIN_SPLIT}_cam_rw\/semantic_gray#g"\
    -e "s#VOC2012#${DATAROOT}\/VOC2012#g"\
    ${TRAIN_DATA_LIST} > ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_rw/list.txt

  python3 pyscripts/inference/prototype_msc.py\
    --data_dir ${HOME}\
    --data_list ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_rw/list.txt\
    --snapshot_dir ${SNAPSHOT_DIR}/stage1\
    --save_dir ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn\
    --kmeans_num_clusters 12,12\
    --label_divisor 2048\
    --cfg_path ${SNAPSHOT_DIR}/config_emb.yaml

  python3 pyscripts/inference/pseudo_inference_crf_msc.py\
    --data_dir ${DATAROOT}\
    --data_list ${TRAIN_DATA_LIST}\
    --snapshot_dir ${SNAPSHOT_DIR}/stage1\
    --save_dir ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn\
    --semantic_memory_dir ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn/semantic_prototype\
    --kmeans_num_clusters 12,12\
    --label_divisor 2048\
    --crf_iter_max 10\
    --crf_pos_w 3\
  ...
  python3 pyscripts/benchmark/benchmark_by_mIoU.py\
    --pred_dir ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn/semantic_gray\
    --gt_dir ${DATAROOT}/VOC2012/segcls\
    --num_classes 21
  ...
  sed -e "s#VOC2012\/cam\/seam_a6_th0.2#`pwd`\/${SNAPSHOT_DIR}\/stage1\/pseudo_labels\/${TRAIN_SPLIT}_cam_nn\/semantic_gray#g"\
    -e "s#VOC2012#${DATAROOT}\/VOC2012#g"\
    ${TRAIN_DATA_LIST} > ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn/list.txt
fi
...
if [ ${IS_TRAIN_CLASSIFIER_1} -eq 1 ]; then
  python3 pyscripts/train/train_classifier.py\
    --data_dir ${HOME}\
    --data_list ${SNAPSHOT_DIR}/stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn/list.txt\
...
if [ ${IS_INFERENCE_CLASSIFIER_1} -eq 1 ]; then
  python3 pyscripts/inference/inference_softmax_crf_msc.py\
    --data_dir ${DATAROOT}\
    --data_list ${TEST_DATA_LIST}\
    --snapshot_dir ${SNAPSHOT_DIR}/softmax_classifier_stage1\
...
# Generate pseudo labels from softmax classifier.
if [ ${IS_ANNOTATION_2} -eq 1 ]; then

  python3 pyscripts/inference/pseudo_softmax.py\
    --data_dir ${DATAROOT}\
    --data_list ${TRAIN_DATA_LIST}\
    --snapshot_dir ${SNAPSHOT_DIR}/softmax_classifier_stage1\
    --save_dir ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_rw\
    --kmeans_num_clusters 1,1\
    --label_divisor 2048\
    --crf_iter_max 10\
    --crf_pos_w 3\
  ...
  python3 pyscripts/benchmark/benchmark_by_mIoU.py\
    --pred_dir ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_rw/semantic_gray\
    --gt_dir ${DATAROOT}/VOC2012/segcls\
    --num_classes 21
  ...
  sed -e "s#VOC2012\/cam\/seam_a6_th0.2#`pwd`\/${SNAPSHOT_DIR}\/softmax_classifier_stage1\/pseudo_labels\/${TRAIN_SPLIT}_cam_rw\/semantic_gray#g"\
    -e "s#VOC2012#${DATAROOT}\/VOC2012#g"\
    ${TRAIN_DATA_LIST} > ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_rw/list.txt

  python3 pyscripts/inference/prototype_msc.py\
    --data_dir ${HOME}\
    --data_list ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_rw/list.txt\
    --snapshot_dir ${SNAPSHOT_DIR}/stage1\
    --save_dir ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn\
    --kmeans_num_clusters 12,12\
    --label_divisor 2048\
    --cfg_path ${SNAPSHOT_DIR}/config_emb.yaml
  ...
  python3 pyscripts/inference/pseudo_inference_crf_msc.py\
    ...
    --snapshot_dir ${SNAPSHOT_DIR}/stage1\
    --save_dir ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn\
    --semantic_memory_dir ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn/semantic_prototype\
    --kmeans_num_clusters 12,12\
    --label_divisor 2048\
    --crf_iter_max 10\
    --crf_pos_w 3\
  ...
  python3 pyscripts/benchmark/benchmark_by_mIoU.py\
    --pred_dir ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn/semantic_gray\
    --gt_dir ${DATAROOT}/VOC2012/segcls\
    --num_classes 21
  ...
  sed -e "s#VOC2012\/cam\/seam_a6_th0.2#`pwd`\/${SNAPSHOT_DIR}\/softmax_classifier_stage1\/pseudo_labels\/${TRAIN_SPLIT}_cam_nn\/semantic_gray#g"\
    -e "s#VOC2012#${DATAROOT}\/VOC2012#g"\
    ${TRAIN_DATA_LIST} > ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn/list.txt
fi


# Train softmax classifier while fix embedding network.
if [ ${IS_TRAIN_CLASSIFIER_2} -eq 1 ]; then
  python3 pyscripts/train/train_classifier.py\
    --data_dir ${HOME}\
    --data_list ${SNAPSHOT_DIR}/softmax_classifier_stage1/pseudo_labels/${TRAIN_SPLIT}_cam_nn/list.txt\
    --snapshot_dir ${SNAPSHOT_DIR}/softmax_classifier_stage2\
    --cfg_path ${SNAPSHOT_DIR}/config_classifier.yaml
fi

# Inference. 
if [ ${IS_INFERENCE_CLASSIFIER_2} -eq 1 ]; then
  python3 pyscripts/inference/inference_softmax_crf_msc.py\
    --data_dir ${DATAROOT}\
    --data_list ${TEST_DATA_LIST}\
    --snapshot_dir ${SNAPSHOT_DIR}/softmax_classifier_stage2\
    --save_dir ${SNAPSHOT_DIR}/softmax_classifier_stage2/results/${INFERENCE_SPLIT}\
    --crf_iter_max 10\
    --crf_pos_w 3\
    --crf_pos_xy_std 1\
    --crf_bi_w 4\
...
# Benchmark.
if [ ${IS_BENCHMARK_CLASSIFIER_2} -eq 1 ]; then
  python3 pyscripts/benchmark/benchmark_by_mIoU.py\
    --pred_dir ${SNAPSHOT_DIR}/softmax_classifier_stage2/results/${INFERENCE_SPLIT}/semantic_gray\
    --gt_dir /home/twke/data/VOCdevkit/VOC2012/segcls\
    --num_classes 21
fi
