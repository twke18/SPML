...
class SegsortSoftmax(nn.Module):
  ...
  def losses(self, datas, targets={}):
    ...
    if self.sem_ann_loss is not None or self.sem_occ_loss is not None:
      ...
      prototypes_with_loc = targets['prototype_with_loc']
      ...
      memory_prototypes = targets.get(
          'memory_prototype', [])
      memory_prototypes_with_loc = targets.get(
          'memory_prototype_with_loc', [])
      ...
      if (memory_prototypes
        ...
        prototypes_with_loc = [prototypes_with_loc]
        prototypes_with_loc += memory_prototypes_with_loc
        ...
        prototypes_with_loc = torch.cat(prototypes_with_loc, dim=0)
      ...
      # Assign image tags by nearest neighbor.
      prototype_semantic_tags = (
        model_utils.gather_multiset_labels_per_batch_by_nearest_neighbor(
            prototypes_with_loc,
            prototypes_with_loc,
      ...
      ignore_mask = (
        torch.max(prototype_semantic_tags, dim=1, keepdim=True)[0] == 0)
      prototype_semantic_tags = prototype_semantic_tags.masked_fill(
          ignore_mask.expand(-1, self.num_classes), 1)
      semantic_tags = torch.index_select(
          prototype_semantic_tags, 0, cluster_indices)
      ...
      if self.sem_ann_loss is not None:
      ...
      if self.sem_occ_loss is not None:
