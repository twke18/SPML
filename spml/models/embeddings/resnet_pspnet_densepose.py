"""Build segmentation model with PSPNet for DensePose."""
...
class ResnetPspnet(ResnetBase):
  ...
  def __init__(self, backbone_depth, strides, dilations, config):
    ...
    self.lfn = LocationColorNetwork(use_color=True, use_location=True,
                                    norm_color=True, smooth_ksize=5)
  ...
  def generate_clusters(self, embeddings,
    ...
    if local_features is not None:
      if semantic_labels is not None:
          ...
          local_features = torch.index_select(
              local_features, 0, valid_pixels)
